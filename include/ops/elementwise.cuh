...
// oneflow-style packed elementwise (their Pack/ApplyGeneric idea): a single
...
#pragma unroll
    for (int j = 0; j < pack_size; ++j) { r.elem[j] = op(pack_in[i].elem[j]...); }
    pack_out[i] = r;
  }
  if (tid < n_tail) { tail_out[tid] = op(tail_in[tid]...); }
}
...
void map_launch(const Op& op, int n, float* out, const In*... in) {
  ...
  if ((aligned_for_pack(out) && ... && aligned_for_pack(in)) && n_pack > 0) {
...
// binary map: out[i] = op(a[i], b[i]); 4-wide packed when 16B-aligned
