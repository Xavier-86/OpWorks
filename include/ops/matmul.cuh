...
#pragma unroll
            for (int i = 0; i < kSub; ++i)
#pragma unroll
                for (int j = 0; j < kSub; ++j) acc[i][j] += a[i] * b[j];
        }
        __syncthreads();
    }
...
inline void mat_mul(const float* A, const float* B, float* C, int M, int N, int K, Epilogue epilogue = {}) {
    launch(detail::mat_mul_kernel<Epilogue>, dim3(detail::blocks_for(K, kMatmulTile), detail::blocks_for(M, kMatmulTile)), dim3(kMatmulBlock, kMatmulBlock), A, B, C, M, N, K, epilogue);
}
