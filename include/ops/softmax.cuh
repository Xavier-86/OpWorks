...
inline void softmax(const float *in, float *out, int rows, int cols, cudaStream_t stream = nullptr) {
    detail::validate_matrix(rows, cols);
    if (rows == 0)
        return;
    detail::require(cols > 0, "softmax requires positive columns for nonempty rows");
    detail::require(in && out, "softmax requires non-null pointers");
    launch_async(stream, detail::softmax_rows_kernel, rows, kThreads, in, out, cols);
}
